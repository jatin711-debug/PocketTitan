...
class TensorAddressTable:
    ...
    def get_tensors_for_layer(self, layer_idx: int) -> List[TensorAddress]:
        ...
        return [t for t in self.tensors.values() if any(pat in t.name for pat in patterns)]
    ...
    def get_tensors_for_expert(self, layer_idx: int, expert_idx: int) -> List[TensorAddress]:
        ...
        return [t for t in layer_tensors if any(pat in t.name for pat in expert_patterns)]
    ...
    def compute_active_params(self) -> int:
        ...
        if (
            not self.metadata.is_moe
            or not self.metadata.num_experts
            or not self.metadata.num_experts_per_tok
        ):
            return total
