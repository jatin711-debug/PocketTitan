...
class PocketTitanModelRunner:
    ...
    def __init__(self, checkpoint_dir: Union[str, Path], device: str = "cuda" if torch.cuda.is_available() else "cpu"):
        ...
        print(f"Loaded PocketTitan Model Metadata (Bits={self.bits}, GroupSize={self.group_size}, Device={self.device})")
    ...
    def inspect_layer_sample(self, layer_idx: int = 0) -> str:
        ...
        shard_file = self.checkpoint_dir / idx.get("weight_map", {}).get(
            f"model.language_model.layers.{layer_idx}.self_attn.q_proj.packed_weight", "model-00001-of-00001.safetensors"
        )
