...
from pockettitan.package.plan import BuildPlan, DenseWorkItem, ExpertWorkItem, PleShardWorkItem
...
class BuildJournal(BaseModel):
    """Resumption state. Written after each item so a crash loses at most one."""

    plan_fingerprint: str
    dense_done: List[str] = Field(default_factory=list)
    expert_done: List[int] = Field(default_factory=list, description="Completed bank record indices")
...
class PackageWriter:
    ...
    def _save_journal(self, journal: BuildJournal) -> None:
        ...
        tmp.write_text(journal.model_dump_json(indent=2), encoding="utf-8")
    ...
    def _write_dense(self, journal: BuildJournal, on_item: Optional[Callable]) -> Tuple[int, int, float]:
        ...
        with open(self.dense_path, "r+b") as blob:
            for item in pending:
                ...
                payload = self._encode_dense(item, tensor)
    ...
    def _encode_dense(self, item: DenseWorkItem, tensor: torch.Tensor) -> bytes:
        if item.bits >= 16:
            raw = tensor.detach().to(torch.float16).cpu().contiguous().view(torch.uint8).numpy().tobytes()
        ...
        result, _, _ = self._quantize(matrix, item.bits, item.group_size, item.symmetric)
        return _sections_to_bytes(result, item.spans, item.address.name)

    def _address_of(self, item: DenseWorkItem):
        return item.address

    def _write_experts(self, journal: BuildJournal, on_item: Optional[Callable]) -> Tuple[int, int, float]:
    ...
    def _encode_expert(self, item: ExpertWorkItem) -> Tuple[bytes, float]:
        ...
        for source in item.expert_slice.projections:
            ...
            peak = max(peak, item_peak)
            payload.extend(
                _sections_to_bytes(result, projection.spans, f"L{item.layer}E{item.expert}.{source.projection}")
            )
            del weight
    ...
    def _write_ple(self, journal: BuildJournal, on_item: Optional[Callable]) -> Tuple[int, int, float]:
        ple = self.plan.ple
        if ple is None:
            return 0, 0, 0.0
    ...
    def _write_metadata(self) -> None:
        """Emit manifest, expert layout, and PLE index."""
        ...
        if manifest.expert_layout is not None:
            ...
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_text(manifest.expert_layout.model_dump_json(indent=2), encoding="utf-8")
        if manifest.ple_index is not None:
            path = self.output_dir / PLE_DIR / PLE_INDEX_NAME
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_text(manifest.ple_index.model_dump_json(indent=2), encoding="utf-8")
