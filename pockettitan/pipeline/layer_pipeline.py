...
class ShardWriter:
    ...
    def finalize(
        self, base_config: Optional[Dict] = None, quant_config: Optional[QuantConfig] = None
    ) -> Dict[str, Any]:
...
class QuantizationPipeline:
    ...
    def __init__(
        ...
        self.shard_writer = ShardWriter(
            output_dir, max_shard_size_mb=budget_config.max_cpu_staging_mb
        )
    ...
    def run(self) -> Dict[str, Any]:
        ...
        self.console.print(
            f"[bold cyan]Starting PocketTitan Quantization Pipeline for {self.model_id_or_path}[/bold cyan]"
        )

        # Build 100% address table
        table = build_tensor_address_table(
            self.model_id_or_path, token=self.token, fast_inspect=False
        )
        ...
        if manifest_mgr.exists():
            ...
            self.console.print(
                f"[yellow]Resuming existing job from manifest ({manifest.completed_tensors}/{manifest.total_tensors} completed)[/yellow]"
            )
        else:
        ...
        self.console.print(
            f"Total Tensors in Index: {total_tensors} | Execution Device: {self.quant_config.device}"
        )
        ...
        with progress:
            main_task = progress.add_task(
                f"[bold green]Overall Progress ({total_tensors} tensors)", total=total_tensors
            )
            ...
            for idx, addr in enumerate(all_tensors, start=1):
                ...
                if rec and rec.status == TensorStatus.COMPLETED:
                    progress.update(
                        main_task,
                        description=f"[green]Skipping completed {addr.name[:25]}...",
                        advance=1,
                    )
                    continue
                ...
                progress.update(
                    main_task,
                    description=f"[blue]Processing ({idx}/{total_tensors}): {addr.name[:32]}...",
                )
                ...
                progress.reset(
                    ...
                    visible=True,
                    description=f"[cyan]Streaming {addr.name[:25]}...",
                )
                ...
                try:
                    if self._should_quantize_tensor(addr.name, addr.shape):
                        ...
                        tensor_data = self.reader.read_tensor(
                            addr, chunk_callback=on_chunk if not self.is_local else None
                        )
                        progress.update(stream_task, visible=False)
        ...
        index_data = self.shard_writer.finalize(
            base_config=base_config, quant_config=self.quant_config
        )
        ...
        self.console.print(
            f"[bold green]Successfully quantized {total_tensors} tensors in {elapsed:.2f}s![/bold green]"
        )
